...
class Person
  ...
  def self.get_students_by_age(age)
    ...
    all_students.select do |student|
      age_at_birthday = today.year - student.date_of_birth.year
      birthday_this_year = Date.new(today.year, student.date_of_birth.month, student.date_of_birth.day)
      age_at_birthday -= 1 if today < birthday_this_year
      puts "#{student.name} (born #{student.date_of_birth}) is #{age_at_birthday} years old."
      age_at_birthday >= age
    end
  end
